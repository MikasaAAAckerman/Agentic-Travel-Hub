...
> "我的多 Agent 旅行规划系统在测试中发现了循环调用和 CLARIFY 泛滥的问题。Orchestrator 会反复派发相同任务给 SubAgent，最严重的一次 RouteAgent 被调用了 8 次。同时 SubAgent 会反复追问已有的信息，CLARIFY 请求高达 9 次。日志噪音太大，肉眼扫半小时才能定位问题。"

### 5.2 设计思路

> "我设计了一套四层渐进式监控体系，每一层都建立在前一层的基础上：
>
> 第一层是结构化日志，通过 MDC 注入 traceId、agentName、round、isRetry 上下文字段，配合 JSON 输出，可以直接用 jq 筛选出所有重试调用。这是最基础的一层，不需要任何额外基础设施。
>
> 第二层是 Micrometer 指标，把日志中的离散事件聚合成可查询的指标——Counter 计数、Histogram 分布、Gauge 瞬时值。通过 Prometheus 抓取，能做数量化分析，比如 p95 轮数、p99 耗时。
>
> 第三层是 OpenTelemetry 链路追踪，每次请求生成完整的调用树，在 Jaeger 上可以看到每个 Agent 的调用时长和依赖关系。循环调用在瀑布图上一目了然。
>
> 第四层是 Grafana 看板 + 告警，设计了请求概览、Agent 分析、异常检测、性能分析四个面板，并配置了循环调用、CLARIFY 泛滥等告警规则。问题发生时自动通知，从被动排查变成主动监控。"

### 5.3 为什么层层递进

> "层层递进的设计是经过深思熟虑的。每一层都是下一层的前置条件：
>
> 没有结构化日志，指标埋点就无法从日志中提取数据；没有指标数据，链路追踪就无法标注异常 Span；没有链路数据，看板就无法展示调用树；没有看板，告警规则就无处配置。
>
> 这种设计还有一个好处：每一层都能独立使用。即使只完成 Layer 1，也能用 jq 快速排查问题。完成 Layer 2 后，就能用 Prometheus 做数量化分析。不需要全部做完才能发挥作用。"

### 5.4 最终效果

> "最终效果是问题排查时间从'肉眼扫半小时日志'降到'1 分钟定位 Span'。系统健康状态实时可视，循环调用、CLARIFY 泛滥等问题能在发生时自动告警。"
