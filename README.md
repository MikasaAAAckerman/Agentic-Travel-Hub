...
**核心价值不仅在"能跑"，还在于经历了五个架构版本的完整迭代**，每个版本都有明确的优化目标和可量化的收益。

---

## 效果演示

> 用户输入自然语言需求，Agent 自主拆解任务、调度领域专家、调用工具、汇总方案。

![旅行规划演示](docs/demo_1080.gif)
...
```
v0 ─────────────→ v1 ───────────────→ v2 ─────────────→ v3 ──────────────────→ v4（当前版本）
while-if-else     手写状态机            多Agent+while     多Agent+Graph嵌套         可观测性全栈监控 ✅
单Agent            单Agent             编排者+多专家       两层Graph                 接入MySQL记录全局聊天记录，用户已完成的推荐计划，待完成计划等
双模型分离          手写Graph思路         模板方法模式        Ali Graph + 记忆压缩      接入PG vector，完善个人标签，个人偏好，反馈回流权重动态变更等
```
...
**可优化点**：1、接入MySQL记录全局聊天记录，用户已完成的推荐计划，待完成计划等；2、接入PG vector，完善个人标签，个人偏好，反馈回流权重动态变更等；3、~~接入 Prometheus + Grafana 监控~~ → 已在 v4 完成 ✅

---

## 架构演进：v3 两层 Graph 嵌套

![v3_整体架构图](docs/v3_整体架构图.jpg)

---

## 最新封版：v4 可观测性全栈监控

![v4_监控设计](docs/v4_监控设计.jpg)

### 四层监控体系

| 层级 | 技术栈 | 职责 | 数据流 |
|------|--------|------|--------|
| **Layer 1** · 结构化日志 | Logback JSON → Logstash → Elasticsearch → Kibana | Agent 执行日志、LLM 请求/响应追踪、错误诊断 | `AgentMDCAspect` → MDC → JSON log → ES → Kibana |
| **Layer 2** · 指标采集 | Micrometer → Actuator → Prometheus → Grafana | 11 个指标（5 Counter + 4 Timer + 2 Gauge），QPS/耗时/错误率 | `AgentMetrics` → `/actuator/prometheus` → Prometheus → Grafana |
| **Layer 3** · 链路追踪 | Micrometer Tracing → OTLP → Jaeger | 完整调用链可视化（Controller → Orchestrator → Planner/Worker → Tool） | `AgentTracing` → OTLP → Jaeger |
| **Layer 4** · 告警通知 | Grafana Alerting + Prometheus Rules | 异常自动告警（高错误率、高重试率、服务宕机） | Prometheus Rules → Grafana AlertManager |

### 架构亮点

- **零侵入埋点**：通过 `AgentMDCAspect`（AOP 切面）统一拦截所有 `Agent.execute()` 方法，业务代码无需手动调用
- **三层关联**：TraceId 贯穿日志、指标、链路，从 Grafana 日志面板可直接跳转 Jaeger 查看完整调用链
- **5 大 Grafana 面板**：请求概览 → 性能分析 → Agent 调用分析 → 异常检测 → ES 日志流
...
| **recursionLimit 源码级踩坑** | 反编译 Spring AI Alibaba Graph Core 定位静默截断问题                          |
| **AOP 零侵入三层监控**        | `AgentMDCAspect` 统一拦截 Agent.execute()，日志/指标/链路追踪三层自动埋点         |
| **可观测性全栈**              | ELK + Prometheus + Grafana + Jaeger，TraceId 贯穿三层，Grafana 五大面板          |
...
- [v0 · while-if-else 手搓 ReAct](docs/设计文档/设计思路/设计思路_v0_ReAct.md)
- [v1 · 手写有向图雏形](docs/设计文档/设计思路/设计思路_v1_Graph形式ReAct.md)
- [v2 · 多Agent 嵌套](docs/设计文档/设计思路/设计思路_v2_多AgentReAct.md)
- [v3 · Graph 形式多 Agent](docs/设计文档/设计思路/设计思路_v3_Graph形式多AgentReAct.md)
